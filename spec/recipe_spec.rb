...
describe Recipe do
    ...
    it "can check needed amounts" do
      @mac_and_cheese.add_ingredient(@cheese, 2)
      expect(@mac_and_cheese.check_amount(@cheese)).to eq("2 oz")

      @mac_and_cheese.add_ingredient(@mac, 8)
      expect(@mac_and_cheese.check_amount(@mac)).to eq("8 oz")

      expect(@mac_and_cheese.check_amount("steak")).to eq(nil)
    end
